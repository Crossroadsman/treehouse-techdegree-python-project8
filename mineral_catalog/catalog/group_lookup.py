...
group_list = Mineral.objects.order_by().values_list(
    'group', flat=True
).distinct()
group_match = 'exact'

category_list = [
    'Amphibole', 'Antimonide', 'Arsenate', 'Arsenic', 'Arsenide', 'Arsenite',
    'Borate', 'Carbonate', 'Chromate', 'Copper', 'Dark mica', 'Feldspar',
    'Feldspathoid', 'Garnet', 'Halide', 'Inosilicate', 'Iodate', 'Manganese',
    'Metals and intermetallic alloys', 'Meteorite', 'Molybdate', 'Native',
    'Nesosilicates', 'Nitrate', 'Organic', 'Oxalate', 'Oxide', 'Phosphate',
    'Pyroxene', 'Rare earth', 'Selenate', 'Selenide', 'Silicate', 'Sulfate',
    'Sulfide', 'Sulfosalt', 'Tectosilicates', 'Tektoborate', 'Telluride',
    'Tellurate', 'Tellurite', 'Titanium', 'Tungstate', 'Uranium', 'Vanadate',
    'Zeolite'
]
category_match = 'contains'

strunz_classification_list = [
    '01', '02', '03', '04', '05', '06', '07', '08', '09'
]  # use start with
strunz_classification_match = 'startswith'

formula_list = [
    'Ag', 'Al', 'As', 'Au', 'B', 'Ba', 'Be', 'Bi', 'Br', 'C', 'Ca', 'Cd', 'Ce',
    'Cl', 'Co', 'Cr', 'Cs', 'Cu', 'F', 'Fe', 'Ge', 'H', 'Hg', 'I', 'K', 'La',
    'Li', 'Mg', 'Mn', 'Mo', 'N', 'Na', 'Nb', 'Nd', 'Ni', 'O', 'P', 'Pb', 'Pd',
    'Pt', 'Rb', 'Re', 'REE', 'S', 'Sb', 'Sc', 'Se', 'Si', 'Sn', 'Sr', 'Ta',
...
crystal_system_list = [
    'Amorphous', 'Coexisting phases', 'Cubic', 'Dipyramidal', 'Ditrigonal',
    'Hexagonal', 'Hexoctahedral', 'Isometric', 'Monoclinic', 'Orthorhombic',
    'Pinacoidal', 'Prismatic', 'Pyramidal', 'Rhombohedral', 'Scalenohedral',
...
class GroupLookup:
    ...
    3. Formula
    ```
    > group = GroupLookup('formula', formula_list, 'regex',
    >     before=r'', after=r'[^a-z]')
    ...
    def make_pattern(self, i):
        if self.before is None or self.after is None:
            raise ValueError(
                'To use regex both `before` and `after` must have valid values'
            )
...
groups = {
    ...
    'category': GroupLookup(
        name='category',
        list=category_list,
        match=category_match
    ),
    'strunz classification': GroupLookup(
        name='strunz_classification',
        list=strunz_classification_list,
        match=strunz_classification_match
    ),
    'formula': GroupLookup(
        name='formula',
        list=formula_list,
        match=formula_match,
        before=formula_regex_before,
        after=formula_regex_after
    ),
    'crystal system': GroupLookup(
        name='crystal_system',
        list=crystal_system_list,
        match=crystal_system_match
    ),
}
